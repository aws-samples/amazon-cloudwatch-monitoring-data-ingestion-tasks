...
```bash
source$ sam build --use-container
```
...
```bash
source$ sam logs -n StockCheckerFunction --stack-name source --tail
```
...
```bash
source$ pip install -r tests/requirements.txt --user
# unit test
source$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
source$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name source
```
